<template>
  <div class="costs-content">
    ...
    <a-alert
      ...
      type="error"
      message="權限不足"
      description="您沒有權限訪問此頁面，僅管理員可以訪問成本管理。"
      show-icon
      closable
    ...
    <template v-if="isAdmin">
      <a-tabs
        ...
        class="costs-tabs"
      >
        <a-tab-pane key="items" tab="成本項目設定" />
        <a-tab-pane key="client" tab="客戶任務成本" />
      </a-tabs>

      <!-- 路由出口 -->
      <div class="costs-router-view">
        <router-view />
      </div>
    </template>
  </div>
...
const getActiveTabFromRoute = () => {
  ...
  return routeTabMap[currentPath] || 'items'
}
