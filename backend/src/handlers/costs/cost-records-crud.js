...
import { invalidateCacheByDataType } from "../../utils/cache-invalidation.js";
...
export async function handleCreateCost(request, env, ctx, requestId, url) {
  ...
  // 失效相關年度報表快取
  await invalidateCacheByDataType(env, "overhead", year).catch((err) => {
    console.warn("[CreateCost] 失效快取失敗:", err);
  });
...
export async function handleUpdateCost(request, env, ctx, requestId, match, url) {
  ...
  // 检查记录是否存在，並獲取年份信息用於失效快取
  const existing = await env.DATABASE.prepare(
    `SELECT overhead_id, year FROM MonthlyOverheadCosts WHERE overhead_id = ? AND is_deleted = 0`
  ).bind(costId).first();
  ...
  // 失效相關年度報表快取
  if (existing?.year) {
    await invalidateCacheByDataType(env, "overhead", existing.year).catch((err) => {
      console.warn("[UpdateCost] 失效快取失敗:", err);
    });
  }
...
export async function handleDeleteCost(request, env, ctx, requestId, match, url) {
  ...
  // 獲取年份信息用於失效快取
  const existing = await env.DATABASE.prepare(
    `SELECT year FROM MonthlyOverheadCosts WHERE overhead_id = ? AND is_deleted = 0`
  ).bind(costId).first();
  ...
  if (existing?.year) {
    await invalidateCacheByDataType(env, "overhead", existing.year).catch((err) => {
      console.warn("[DeleteCost] 失效快取失敗:", err);
    });
  }
